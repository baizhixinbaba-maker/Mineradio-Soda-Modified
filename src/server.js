...
const { prepareSodaBridgeDeployment } = require('./desktop/soda-bridge-deployment');
...
const SODA_API_BASE_URL = 'https://api.qishui.com';
const SODA_VERSION_NAME = '3.5.2';
const SODA_VERSION_CODE = '30502';
const SODA_RUNTIME_STATE_DIR = process.env.SODA_RUNTIME_STATE_DIR || path.join(__dirname, '.soda-runtime');
const SODA_BRIDGE_PORT = Number(process.env.SODA_BRIDGE_PORT || 17891);
const SODA_BRIDGE_TOKEN_FILE = path.join(SODA_RUNTIME_STATE_DIR, 'bridge-token');
function loadSodaBridgeToken() {
  if (process.env.SODA_BRIDGE_TOKEN) return process.env.SODA_BRIDGE_TOKEN;
  try {
    const saved = fs.readFileSync(SODA_BRIDGE_TOKEN_FILE, 'utf8').trim();
    if (/^[a-f0-9]{32,}$/i.test(saved)) return saved;
  } catch (_) {}
  const token = crypto.randomBytes(24).toString('hex');
  try {
    fs.mkdirSync(SODA_RUNTIME_STATE_DIR, { recursive: true });
    fs.writeFileSync(SODA_BRIDGE_TOKEN_FILE, token, { mode: 0o600 });
  } catch (_) {}
  return token;
}
const SODA_BRIDGE_TOKEN = loadSodaBridgeToken();
...
const weatherReverseGeocodeCache = new Map();
...
let sodaSessionRevision = 0;
let sodaPlaylistsCache = { revision: -1, expiresAt: 0, raw: [] };
let sodaLikedTracksCache = { revision: -1, expiresAt: 0, playlistId: '', ids: new Set() };
function saveSodaCookie(c) {
  sodaCookie = normalizeCookieHeader(c) || rawCookieFallback(c);
  sodaSessionRevision++;
  sodaPlaylistsCache = { revision: -1, expiresAt: 0, raw: [] };
  sodaLikedTracksCache = { revision: -1, expiresAt: 0, playlistId: '', ids: new Set() };
  try { fs.writeFileSync(SODA_COOKIE_FILE, sodaCookie, { mode: 0o600 }); } catch (e) {}
}
...
function serveStatic(res, filePath) {
  ...
  fs.readFile(filePath, (err, data) => {
    ...
    if (normalized.endsWith('/soda-login/assets/login-7a4b1afd.js')) {
      data = Buffer.from(data.toString('utf8')
        .replace('version_code:"2.1.0"', 'version_code:"3.5.2"')
        .replace(
          'async function o(){n.value=void 0,r.value=!1,s.value=!0;let l=!1;try{',
          'async function o(){n.value=void 0,r.value=!1,s.value=!0;let l=!1,v=0;try{',
        )
        .replace(
          'const a=await xs(),u="https://luna-pc.bytedance.net/login",c=await a.qrcodeLogin.getQrcode({next:u})',
          'const a=await xs(),u="https://api.qishui.com",c=window.__mineradioSodaQrResponse=await a.qrcodeLogin.getQrcode({next:u,extra_params:{need_logo:!1,need_short_url:!1,is_new_login:"1",is_from_ttaccountsdk:1,is_from_iesaccountsaas:1}})',
        )
        .replace('b.user_data.new_user?1:0', 'b.user_data&&b.user_data.new_user?1:0')
        // Keep the QR token stable. The old embedded page recreated its QR
        // after a transient polling error, while Mineradio still displayed the
        // previous canvas to the user.
        .replace(
          '}catch(b){return ce(le.applog).report({event:"login_error",params:{...Oe(),type:"check_qrcode_status",login_method:"qishui",error_code:b.error_code,fail_info:b.description}}),o()}await to(1e3)}}catch(a){',
          '}catch(b){ce(le.applog).report({event:"login_error",params:{...Oe(),type:"check_qrcode_status",login_method:"qishui",error_code:b.error_code,fail_info:b.description}}),n.value="二维码状态检查失败，正在重试";await to(1500);continue}await to(1e3)}}catch(a){',
        )
        .replace(
          'a.qrcodeLogin.checkQrconnect({token:c.token,next:u})',
          'a.qrcodeLogin.checkQrconnect({token:c.token,next:u,extra_params:{is_from_ttaccountsdk:1,is_from_iesaccountsaas:1}})',
        )
        .replace(
          'const b=await a.qrcodeLogin.checkQrconnect({token:c.token,next:u,extra_params:{is_from_ttaccountsdk:1,is_from_iesaccountsaas:1}});if(',
          'const b=await a.qrcodeLogin.checkQrconnect({token:c.token,next:u,extra_params:{is_from_ttaccountsdk:1,is_from_iesaccountsaas:1}});window.__mineradioSodaQrCheckResponse=b;v+=1;if(',
        )
        .replace(
          'continue}await to(1e3)}}catch(a){',
          'continue}await to(v>=180?5e3:v>=60?3e3:1e3)}}catch(a){',
        ));
    } else if (normalized.endsWith('/soda-login/assets/index-d195ae8d.js')) {
      data = Buffer.from(data.toString('utf8')
        .replace(/2\.0\.10-beta\.0/g, '2.4.13')
        .replace('passport_jssdk_type:"pro"', 'passport_jssdk_type:"normal"')
        .replace(
          'passport_verify:cee,request_host:',
          'passport_verify:cee,p_js_v:"2.4.13",p_js_t:"pro",p_zt:"3.3.5",p_ver:"1.0.29",p_bd:"1.0.0.41",is_from_ttaccountsdk:1,is_from_iesaccountsaas:1,request_host:',
        )
        // The bundled account SDK is older than Soda 3.5.2. Keep its
        // credentialed request layer but align QR get/check semantics.
        .replace(
          'e.prototype.getQrcode=function(t){var r=t.next,n=t.extra_params,i=n===void 0?{}:n;return this.request({url:"".concat(ue.GET_QRCODE,"?next=").concat(encodeURIComponent(r)),method:"GET",data:i})},e.prototype.checkQrconnect=function(t){var r=t.next,n=t.token,i=t.extra_params;return this.request({url:"".concat(ue.CHECK_QRCONNECT,"?next=").concat(encodeURIComponent(r)),method:"GET",data:cs({token:n},i)})}',
          'e.prototype.getQrcode=function(t){var r=t.next,n=t.extra_params,i=n===void 0?{}:n;return this.request({url:"".concat(ue.GET_QRCODE,"?next=").concat(encodeURIComponent(r)),method:"GET",data:cs({need_logo:!1,need_short_url:!1,is_new_login:"1"},i)})},e.prototype.checkQrconnect=function(t){var r=t.next,n=t.token,i=t.extra_params,o=i===void 0?{}:i;return this.request({url:ue.CHECK_QRCONNECT,method:"POST",data:cs({need_logo:!1,need_short_url:!1,is_frontier:!0,token:n,is_new_login:"1",next:r},o)})}',
        ));
    }
    const headers = { 'Content-Type': MIME[ext] || 'text/plain' };
    if (normalized.includes('/soda-login/')) headers['Cache-Control'] = 'no-store';
...
async function reverseWeatherLocation(latitude, longitude) {
  const lat = clampNumber(latitude, -90, 90, NaN);
  const lon = clampNumber(longitude, -180, 180, NaN);
  if (!Number.isFinite(lat) || !Number.isFinite(lon)) throw new Error('WEATHER_REVERSE_LOCATION_INVALID');
  const cacheKey = `${lat.toFixed(3)},${lon.toFixed(3)}`;
  const cached = weatherReverseGeocodeCache.get(cacheKey);
  if (cached && cached.expiresAt > Date.now()) return cached.value;
  const url = new URL(WEATHER_REVERSE_GEOCODE_URL);
  url.searchParams.set('latitude', String(lat));
  url.searchParams.set('longitude', String(lon));
  url.searchParams.set('localityLanguage', 'zh');
  const data = await requestJson(url.toString(), { headers: { 'User-Agent': 'Mineradio/1.1 local weather' } });
  const city = String(data && (data.city || data.locality || data.principalSubdivision) || '').trim();
  if (!city) throw new Error('WEATHER_REVERSE_LOCATION_EMPTY');
  const value = {
    city,
    locality: String(data && data.locality || '').trim(),
    region: String(data && data.principalSubdivision || '').trim(),
    country: String(data && data.countryName || '').trim(),
  };
  weatherReverseGeocodeCache.set(cacheKey, { value, expiresAt: Date.now() + 6 * 60 * 60 * 1000 });
  if (weatherReverseGeocodeCache.size > 128) {
    for (const [key, entry] of weatherReverseGeocodeCache) if (!entry || entry.expiresAt <= Date.now()) weatherReverseGeocodeCache.delete(key);
  }
  return value;
}

function fetchWindowsWeatherLocation() {
  if (process.platform !== 'win32') return Promise.reject(new Error('WINDOWS_LOCATION_UNSUPPORTED'));
  const script = [
    ...
    '$operation = $geo.GetGeopositionAsync([TimeSpan]::FromSeconds(0), [TimeSpan]::FromSeconds(15))',
    '$type = [Windows.Devices.Geolocation.Geoposition,Windows.Devices.Geolocation,ContentType=WindowsRuntime]',
    ...
    "if (-not $task.Wait(17000)) { throw 'WINDOWS_LOCATION_TIMEOUT' }",
    '$position = $task.Result.Coordinate.Point.Position',
    '[pscustomobject]@{ latitude = [double]$position.Latitude; longitude = [double]$position.Longitude; accuracyMeters = [double]$task.Result.Coordinate.Accuracy } | ConvertTo-Json -Compress',
  ].join('; ');
  return new Promise((resolve, reject) => {
    let child = null;
    let settled = false;
    ...
    const timer = setTimeout(() => {
      if (child) child.kill();
      finish(new Error('WINDOWS_LOCATION_TIMEOUT'));
    }, 19000);
    try {
      ...
      child.once('close', code => {
        if (code !== 0) return finish(new Error(stderr.trim() || `WINDOWS_LOCATION_EXIT_${code}`));
        try {
          ...
          if (!Number.isFinite(latitude) || !Number.isFinite(longitude) || Math.abs(latitude) > 90 || Math.abs(longitude) > 180 || (!latitude && !longitude)) {
            throw new Error('WINDOWS_LOCATION_INVALID');
          }
          finish(null, {
            provider: 'windows-location',
            latitude,
            longitude,
            accuracyMeters: Number(data.accuracyMeters) || 0,
...
function sodaHasLogin() {
  const cookies = sodaCookieObject();
  return !!(cookies.sessionid || cookies.sessionid_ss || cookies.sid_tt);
}

function sodaCoverUrl(value) {
  if (!value) return '';
  ...
  if (!uri || first.includes(uri)) return first;

  // Luna's URLInfo requires an image-processing suffix. A bare urls[0] + uri
  // address returns 400 from douyinpic.com, while the official client uses c5.
  const base = first.replace(/\/+$/, '/') + uri.replace(/^\/+/, '');
  if (/[~?]/.test(uri)) return base;
  const prefix = String(value.template_prefix || '').trim();
  const suffix = prefix
    ? `${prefix}-crop-center:400:400.jpg`
    : 'c5_400x400.jpg';
  return `${base}~${suffix}`;
}

async function sodaRequest(pathname, options) {
  if (!sodaHasLogin()) {
    const err = new Error('SODA_LOGIN_REQUIRED');
    err.code = 'SODA_LOGIN_REQUIRED';
    throw err;
  }
  options = options || {};
  const target = new URL(pathname, SODA_API_BASE_URL);
  const query = Object.assign({
    aid: '386088',
    device_platform: 'pc',
    version_name: SODA_VERSION_NAME,
  ...
  const headers = {
    'User-Agent': 'SodaMusic/3.5.2 (Windows NT 10.0; Win64; x64) Electron/36.4.0',
    Accept: 'application/json, text/plain, */*',
    Referer: 'https://luna-pc.bytedance.net/',
    Cookie: sodaCookie,
  };
  if (body) {
  ...
  const text = await requestText(target.toString(), {
    method: options.method || 'GET',
    headers,
  }, body);
  const data = parseJSONText(text);
  const statusCode = Number(data && data.status_code || 0);
  if (statusCode && statusCode !== 0) {
    const message = data && data.status_info && data.status_info.status_msg || data && data.message || 'SODA_API_FAILED';
...
function compareSodaVersions(left, right) {
...
function findSodaNativeRuntime() {
  const roots = [
    process.env.SODA_MUSIC_HOME,
    'D:\\汽水音乐\\Soda Music',
    path.join(process.env.LOCALAPPDATA || '', 'SodaMusic'),
    path.join(process.env.PROGRAMFILES || '', 'Soda Music'),
  ].filter(Boolean);
  const candidates = [];
  for (const root of roots) {
    const packageRoot = path.join(root, 'Packages');
    try {
      fs.readdirSync(packageRoot, { withFileTypes: true }).forEach(entry => {
        if (!entry.isDirectory()) return;
        const unpacked = path.join(packageRoot, entry.name, 'app.asar.unpacked');
        if (['ttnet.node', 'bdticket.node', 'bdms.node'].every(file => fs.existsSync(path.join(unpacked, file)))) {
          candidates.push({ root, version: entry.name, unpacked });
        }
      });
    } catch (_) {}
  }
  candidates.sort((a, b) => compareSodaVersions(b.version, a.version));
  const selected = candidates[0];
  if (!selected) throw new Error('SODA_OFFICIAL_RUNTIME_NOT_FOUND');

  const dllDirs = [
    selected.root,
    ...(() => {
      try {
        return fs.readdirSync(selected.root, { withFileTypes: true })
          .filter(entry => entry.isDirectory())
          .map(entry => path.join(selected.root, entry.name));
      } catch (_) {
        return [];
      }
    })(),
  ];
  const dllDir = dllDirs.find(dir => fs.existsSync(path.join(dir, 'sscronet.dll')));
  if (!dllDir) throw new Error('SODA_TTNET_DLL_NOT_FOUND');
  return { ...selected, dllDir };
}

function sodaTtnetRequest(runtime, options) {
  return new Promise((resolve, reject) => {
    const chunks = [];
    let response = null;
    const timeout = setTimeout(() => reject(new Error('SODA_TTNET_TIMEOUT')), 30000);
    const finish = (error, value) => {
    ...
    try {
      runtime.ttnet.request({
        url: String(options.url),
        method: options.method || 'GET',
        headers: Object.fromEntries(Object.entries(options.headers || {}).map(([key, value]) => [key, Array.isArray(value) ? value.map(String) : [String(value)]])),
        ...
        onEvent(event) {
          if (event.type === 'response') {
            ...
            if (!response) finish(new Error('SODA_TTNET_EMPTY_RESPONSE'));
            else finish(null, {
            ...
            finish(event.error || new Error('SODA_TTNET_REQUEST_FAILED'));
          }
        },
      });
    } catch (error) {
...
async function ensureSodaNativeRuntime() {
  if (sodaNativeRuntime.ready) return sodaNativeRuntime;
  if (sodaNativeRuntime.initializing) return sodaNativeRuntime.initializing;
  sodaNativeRuntime.initializing = (async () => {
    const install = findSodaNativeRuntime();
    process.env.PATH = `${install.dllDir}${path.delimiter}${process.env.PATH || ''}`;
    const ttnet = require(path.join(install.unpacked, 'ttnet.node'));
    const bdticket = require(path.join(install.unpacked, 'bdticket.node'));
    const bdms = require(path.join(install.unpacked, 'bdms.node'));
    const deviceId = crypto.createHash('sha256').update(sodaCookie || 'mineradio-soda').digest('hex').slice(0, 32);
    fs.mkdirSync(SODA_RUNTIME_STATE_DIR, { recursive: true });
    bdticket.startBDTicket(SODA_RUNTIME_STATE_DIR);
    bdms.init({ deviceId });
    ttnet.initTTNet({
      storagePath: path.join(SODA_RUNTIME_STATE_DIR, 'ttnet'),
      appId: '386088',
      appName: 'luna_pc',
      deviceId,
      versionCode: SODA_VERSION_CODE,
      deviceType: 'Windows',
      channel: 'official',
      devicePlatform: 'windows',
      uuid: deviceId,
      updateVersionCode: SODA_VERSION_CODE,
      tncHostFirst: 'tnc3-bjlgy.zijieapi.com',
      tncHostSecond: 'tnc3-alisc1.zijieapi.com',
      tncHostThird: 'tnc3-aliec2.zijieapi.com',
      isMainProcess: '1',
      httpDns: 'dig.bdurl.net',
      domainNetlog: 'crash.snssdk.com',
      domainBoe: '.boe-gateway.byted.org',
      domainDefaultJson: {
        message: 'success',
        opaque_data_enabled: 1,
        data: { http_dns_enabled: 1, ttnet_http_dns_enabled: 1, ttnet_tt_http_dns: 1 },
      },
      tncHeaders: [],
      tncQueries: {},
      tncLoadFlags: 0,
      httpDnsLoadFlags: 0,
      useDomesticStoreRegion: true,
      useInjectMode: true,
    });
    Object.assign(sodaNativeRuntime, {
      ready: true,
      error: '',
      root: install.unpacked,
    ...
    return sodaNativeRuntime;
  })().catch(error => {
    sodaNativeRuntime.error = error.message || String(error);
    sodaNativeRuntime.initializing = null;
    throw error;
  });
  return sodaNativeRuntime.initializing;
}

let sodaBridgeStartPromise = null;
let sodaBridgeDeploymentPromise = null;
let sodaBridgeProcess = null;
let sodaBridgeLastFailure = { at: 0, error: '' };
const SODA_BRIDGE_RETRY_COOLDOWN_MS = 15000;
const SODA_BRIDGE_PROFILE_DIR = path.join(SODA_RUNTIME_STATE_DIR, 'bridge-profile');
const sodaAudioDecryptions = new Map();

function rememberSodaAudioDecryption(url, key) {
  if (!url || !/^[0-9a-f]{32}$/i.test(String(key || ''))) return;
  const now = Date.now();
  for (const [candidate, entry] of sodaAudioDecryptions) {
    if (!entry || entry.expiresAt <= now) sodaAudioDecryptions.delete(candidate);
  }
  sodaAudioDecryptions.set(String(url), { key: String(key), expiresAt: now + 12 * 60 * 1000, buffer: null, contentType: 'audio/mp4' });
}

function sodaArchiveContainsBridge(archive) {
  const previous = process.noAsar;
  try {
    process.noAsar = true;
    return fs.readFileSync(archive).includes(Buffer.from('MINERADIO_SODA_BRIDGE_PORT'));
  } catch (_) {
    return false;
  } finally {
...
function findSodaBridgeExecutable() {
  const configuredExecutable = String(process.env.SODA_BRIDGE_EXECUTABLE || '').trim();
  if (configuredExecutable) {
    const configuredArchive = String(process.env.SODA_BRIDGE_ARCHIVE || '').trim();
    const archives = [
      configuredArchive,
      path.join(path.dirname(configuredExecutable), 'resources', 'app.asar'),
    ].filter(Boolean);
    if (fs.existsSync(configuredExecutable) && archives.some(sodaArchiveContainsBridge)) return configuredExecutable;
  }
  const roots = [
    process.env.SODA_MUSIC_HOME,
    'D:\\汽水音乐\\Soda Music',
  ...
  for (const root of roots) {
    try {
      fs.readdirSync(root, { withFileTypes: true })
        ...
        .forEach(entry => candidates.push({ root, version: entry.name }));
    } catch (_) {}
  }
  candidates.sort((left, right) => compareSodaVersions(right.version, left.version));
  for (const candidate of candidates) {
    const executable = path.join(candidate.root, candidate.version, 'SodaMusic.exe');
    const archive = path.join(candidate.root, candidate.version, 'resources', 'app.asar');
    if (!fs.existsSync(executable) || !fs.existsSync(archive)) continue;
    if (sodaArchiveContainsBridge(archive)) return executable;
  }
  const deploymentError = String(process.env.SODA_BRIDGE_DEPLOYMENT_ERROR || '').trim();
  if (/^SODA_DEPLOY_[A-Z0-9_]+$/.test(deploymentError)) throw new Error(deploymentError);
  throw new Error('SODA_BRIDGE_RUNTIME_NOT_PATCHED');
}

async function deploySodaBridgeRuntime() {
  if (sodaBridgeDeploymentPromise) return sodaBridgeDeploymentPromise;
  sodaBridgeDeploymentPromise = (async () => {
    const result = await prepareSodaBridgeDeployment({
      userData: path.dirname(SODA_RUNTIME_STATE_DIR),
      env: process.env,
    });
    ...
    const code = String(result && result.error || 'SODA_DEPLOY_FAILED');
    process.env.SODA_BRIDGE_DEPLOYMENT_ERROR = code;
    throw new Error(code);
  })().finally(() => { sodaBridgeDeploymentPromise = null; });
  return sodaBridgeDeploymentPromise;
}

async function resolveSodaBridgeExecutable() {
  try {
    return findSodaBridgeExecutable();
  } catch (_) {
    await deploySodaBridgeRuntime();
    return findSodaBridgeExecutable();
  }
}

function stopSodaBridgeProcess(child) {
  if (!child || (sodaBridgeProcess && sodaBridgeProcess !== child)) return;
  if (sodaBridgeProcess === child) sodaBridgeProcess = null;
  try {
    if (!child.killed) child.kill();
  } catch (_) {}
}

function sodaBridgeRequest(pathname, method, payload, timeoutMs) {
  return new Promise((resolve, reject) => {
    const body = payload == null ? '' : JSON.stringify(payload);
    const req = http.request({
      host: '127.0.0.1',
      port: SODA_BRIDGE_PORT,
      path: pathname,
      method: method || 'GET',
      headers: {
        'x-mineradio-soda-bridge': SODA_BRIDGE_TOKEN,
      ...
      res.on('end', () => {
        ...
        catch (_) { return reject(new Error('SODA_BRIDGE_RESPONSE_INVALID')); }
        if (res.statusCode < 200 || res.statusCode >= 300 || !data.ok) return reject(new Error(data.error || `SODA_BRIDGE_HTTP_${res.statusCode}`));
        resolve(data);
      });
    });
    req.once('timeout', () => req.destroy(new Error('SODA_BRIDGE_TIMEOUT')));
...
async function ensureSodaBridge() {
  try {
    await sodaBridgeRequest('/health', 'GET', null, 800);
    return;
  } catch (_) {}
  if (sodaBridgeStartPromise) return sodaBridgeStartPromise;
  if (sodaBridgeLastFailure.at && Date.now() - sodaBridgeLastFailure.at < SODA_BRIDGE_RETRY_COOLDOWN_MS) {
    throw new Error(sodaBridgeLastFailure.error || 'SODA_BRIDGE_RECENTLY_FAILED');
  }
  sodaBridgeStartPromise = (async () => {
    const executable = await resolveSodaBridgeExecutable();
    fs.mkdirSync(SODA_BRIDGE_PROFILE_DIR, { recursive: true });
    const child = spawn(executable, [`--user-data-dir=${SODA_BRIDGE_PROFILE_DIR}`], {
      ...
      env: {
        ...process.env,
        MINERADIO_SODA_BRIDGE_PORT: String(SODA_BRIDGE_PORT),
        MINERADIO_SODA_BRIDGE_TOKEN: SODA_BRIDGE_TOKEN,
        MINERADIO_SODA_BRIDGE_ONLY: '1',
      },
    });
    sodaBridgeProcess = child;
    child.once('exit', () => {
      if (sodaBridgeProcess === child) sodaBridgeProcess = null;
    });
    let lastError = null;
    try {
      for (let attempt = 0; attempt < 18; attempt++) {
        ...
        try {
          await sodaBridgeRequest('/health', 'GET', null, 900);
          sodaBridgeLastFailure = { at: 0, error: '' };
          return;
        } catch (error) {
          lastError = error;
        }
...
async function sodaNativePlayerRequest(trackId, vid, quality) {
  if (!sodaHasLogin()) throw new Error('SODA_LOGIN_REQUIRED');
  await ensureSodaBridge();
  return sodaBridgeRequest('/player', 'POST', {
    id: String(trackId),
    vid: String(vid || ''),
...
async function sodaNativeLyricRequest(trackId) {
  if (!sodaHasLogin()) throw new Error('SODA_LOGIN_REQUIRED');
  await ensureSodaBridge();
  return sodaBridgeRequest('/lyric', 'POST', {
    id: String(trackId),
    cookie: sodaCookie,
  }, 40000);
}
...
function sodaGet(pathname, params) {
  return sodaRequest(pathname, { method: 'GET', params });
}

function sodaPost(pathname, body, params) {
  return sodaRequest(pathname, { method: 'POST', params, body });
}

function mapSodaPlaylist(raw) {
  raw = raw || {};
  const owner = raw.owner || raw.user || {};
  const sodaType = Number(raw.type || raw.playlist_type || raw.special_type || 0) || 0;
  const isLiked = sodaType === 1 || raw.is_liked === true || raw.is_liked_playlist === true;
  return {
    provider: 'soda',
    source: 'soda',
    type: 'soda',
    id: String(raw.id || raw.playlist_id || ''),
    name: String(raw.title || raw.name || '汽水音乐歌单'),
    cover: sodaCoverUrl(raw.url_cover || raw.cover || raw.cover_url),
    trackCount: Number(raw.count_tracks || raw.track_count || raw.count || 0) || 0,
    creator: String(owner.nickname || raw.creator_name || '汽水音乐'),
    sodaType,
    specialType: isLiked ? 5 : (Number(raw.special_type) || 0),
    isLiked,
    subscribed: !!(raw.subscribed || raw.is_subscribed),
  };
}
...
function mapSodaTrack(raw) {
  ...
  const wrapper = entity.track_wrapper || entity.trackWrapper || raw.track_wrapper || {};
  const track = wrapper.track || entity.track || raw.track || raw;
  const artists = Array.isArray(track.artists) ? track.artists : (Array.isArray(track.artist) ? track.artist : []);
  const artist = artists.map(item => item && (item.name || item.nickname || item.title)).filter(Boolean).join(' / ') ||
    String(track.artist_name || track.artist || '');
  const album = track.album || {};
  const state = track.state || raw.state || {};
  const vid = String(track.vid || raw.vid || '');
  return {
    provider: 'soda',
    source: 'soda',
    type: 'soda',
    id: String(track.id || track.track_id || raw.id || ''),
    name: String(track.title || track.name || '未知歌曲'),
    artist,
    artists: artists.map(item => ({ name: String(item && (item.name || item.nickname || item.title) || '') })).filter(item => item.name),
    album: String(album.title || album.name || track.album_name || ''),
    cover: sodaCoverUrl(track.url_cover || track.cover || album.url_cover || album.cover),
    duration: Number(track.duration || track.duration_ms || 0) || 0,
    vid,
    mediaType: String(track.media_type || raw.media_type || 'track'),
    isCollected: !!(state.is_collected || state.isCollected),
    playable: !!vid,
  };
}

async function loadSodaPlaylists(force) {
  const now = Date.now();
  if (!force && sodaPlaylistsCache.revision === sodaSessionRevision && sodaPlaylistsCache.expiresAt > now) {
    return sodaPlaylistsCache.raw.slice();
  }
  ...
  let page = 0;
  do {
    const data = await sodaGet('/luna/pc/me/playlist', { cursor, count: 50 });
    const entries = Array.isArray(data.playlists) ? data.playlists : [];
    entries.forEach(item => {
      const id = String(item && (item.id || item.playlist_id) || '');
      if (!id || seen.has(id)) return;
      seen.add(id);
      raw.push(item);
    });
    const total = Number(data.total_num || 0) || 0;
    const hasMore = data.has_more === true || (total > 0 && raw.length < total);
    cursor = String(data.next_cursor || (hasMore ? raw.length : ''));
    page++;
    if (!hasMore || !cursor || page >= 30) break;
  } while (true);

  sodaPlaylistsCache = {
    revision: sodaSessionRevision,
...
function sodaLoginInfoFromPlaylists(rawPlaylists) {
  const first = Array.isArray(rawPlaylists) ? rawPlaylists[0] : null;
  const owner = first && (first.owner || first.user) || {};
  return {
    provider: 'soda',
    loggedIn: true,
    version: SODA_VERSION_NAME,
    userId: String(owner.id || owner.user_id || ''),
    nickname: String(owner.nickname || '汽水音乐用户'),
    avatar: sodaCoverUrl(owner.medium_avatar_url || owner.avatar_url || owner.avatar),
    playlistCount: Array.isArray(rawPlaylists) ? rawPlaylists.length : 0,
  };
}

async function getSodaLoginInfo() {
  if (!sodaHasLogin()) return { provider: 'soda', loggedIn: false, version: SODA_VERSION_NAME };
  try {
    return sodaLoginInfoFromPlaylists(await loadSodaPlaylists());
  } catch (err) {
    return { provider: 'soda', loggedIn: false, version: SODA_VERSION_NAME, stale: true, error: err.message };
  }
}

async function handleSodaUserPlaylists() {
  const rawPlaylists = await loadSodaPlaylists(true);
  const playlists = rawPlaylists.map(mapSodaPlaylist).filter(item => item.id);
  const likedPlaylist = playlists.find(item => item.isLiked);
  return Object.assign(sodaLoginInfoFromPlaylists(rawPlaylists), {
    provider: 'soda',
    loggedIn: true,
    playlists,
    likedPlaylistId: likedPlaylist ? likedPlaylist.id : '',
  });
}

async function loadSodaPlaylistTracks(id) {
  const playlistId = String(id || '').trim();
  if (!playlistId) return { playlist: null, tracks: [] };
  const tracks = [];
  const seen = new Set();
  let playlistRaw = null;
  let cursor = '';
  let page = 0;
  do {
    const data = await sodaGet('/luna/pc/playlist/detail', { playlist_id: playlistId, count: 100, cursor });
    if (!playlistRaw && data.playlist) playlistRaw = data.playlist;
    const rawTracks = Array.isArray(data.tracks) && data.tracks.length
      ? data.tracks
      : (Array.isArray(data.media_resources) ? data.media_resources : []);
    rawTracks.forEach(raw => {
      const track = mapSodaTrack(raw);
    ...
    page++;
    if (!data.has_more || !cursor || page >= 100) break;
  } while (true);
  return { playlist: mapSodaPlaylist(playlistRaw || { id: playlistId }), tracks };
}

async function handleSodaPlaylistTracks(id) {
  const playlistId = String(id || '').trim();
  if (!playlistId) return { provider: 'soda', tracks: [], error: 'MISSING_SODA_PLAYLIST_ID' };
  const result = await loadSodaPlaylistTracks(playlistId);
  return { provider: 'soda', loggedIn: true, playlist: result.playlist, tracks: result.tracks };
}

async function getSodaLikedTrackIds(force) {
  const now = Date.now();
  if (!force && sodaLikedTracksCache.revision === sodaSessionRevision && sodaLikedTracksCache.expiresAt > now) {
    return sodaLikedTracksCache.ids;
  }
  const playlists = (await loadSodaPlaylists()).map(mapSodaPlaylist);
  const liked = playlists.find(item => item.isLiked);
  const tracks = liked ? (await loadSodaPlaylistTracks(liked.id)).tracks : [];
  sodaLikedTracksCache = {
    revision: sodaSessionRevision,
    expiresAt: now + 30 * 1000,
    playlistId: liked ? liked.id : '',
    ids: new Set(tracks.map(track => String(track.id))),
  };
  return sodaLikedTracksCache.ids;
}
...
async function handleSodaSearch(keywords, limit) {
  ...
  if (!q) return [];
  const count = Math.max(1, Math.min(50, Number(limit) || 18));
  const data = await sodaGet('/luna/pc/search/track', {
    q,
  ...
  const songs = [];
  const seen = new Set();
  (Array.isArray(data.result_groups) ? data.result_groups : []).forEach(group => {
    (Array.isArray(group && group.data) ? group.data : []).forEach(item => {
      const track = mapSodaTrack(item);
      if (!track.id || seen.has(track.id)) return;
      seen.add(track.id);
      songs.push(track);
    });
...
function sodaPlayerUrlFromResponse(data) {
  const info = data && (data.track_player || data.player_info || (Array.isArray(data.player_infos) && data.player_infos[0]) || data);
  return String(info && (info.url_player_info || info.url || info.play_url) || '');
}

function sodaBridgeDeploymentMessage(code) {
  const messages = {
    SODA_DEPLOY_NOT_INSTALLED: '未检测到受支持的汽水音乐客户端，请先安装官方客户端后重试',
    SODA_DEPLOY_CLIENT_RUNNING: '请先退出官方汽水音乐客户端，再重试播放',
    SODA_DEPLOY_VERSION_UNSUPPORTED: '当前汽水音乐客户端版本暂不受支持，未修改任何客户端文件',
    SODA_DEPLOY_SOURCE_HASH_MISMATCH: '汽水音乐客户端文件已更新或被修改，未修改任何客户端文件',
    SODA_DEPLOY_TARGET_LOCKED: '汽水音乐客户端文件正在使用，请退出官方客户端后重试',
    SODA_DEPLOY_BACKUP_CONFLICT: '检测到不匹配的汽水音乐备份，已停止自动部署',
    SODA_DEPLOY_RECOVERY_FAILED: '汽水音乐 bridge 恢复失败，客户端文件未继续修改',
    SODA_DEPLOY_ROLLBACK_FAILED: '汽水音乐 bridge 部署未完成，请检查部署状态文件',
  };
  return messages[code] || `Soda bridge setup failed: ${code}`;
}

async function handleSodaSongUrl(id, vid, quality) {
  const trackId = String(id || '').trim();
  if (!trackId) return { provider: 'soda', url: '', playable: false, error: 'MISSING_SODA_TRACK_ID' };
  try {
    const data = await sodaNativePlayerRequest(trackId, vid, quality);
    const url = sodaPlayerUrlFromResponse(data);
    if (url) rememberSodaAudioDecryption(url, data && data.decryptionKey);
    return {
      provider: 'soda',
      id: trackId,
      url,
      playable: !!url,
      expireAt: Number(data && data.expire_at || 0) || 0,
      level: String(data && data.level || 'standard'),
      quality: String(data && data.sourceQuality || ''),
      br: Number(data && data.br) || 0,
      reason: url ? '' : 'url_unavailable',
      message: url ? '' : '汽水音乐当前未提供可播放地址',
      error: url ? '' : 'SODA_PLAYER_URL_UNAVAILABLE',
    };
  } catch (error) {
    const code = String(error && error.message || 'SODA_PLAYER_URL_UNAVAILABLE');
...
async function handleSodaLyric(id) {
  const trackId = String(id || '').trim();
  if (!trackId) return { provider: 'soda', lyric: '', tlyric: '', yrc: '', error: 'MISSING_SODA_TRACK_ID' };
  const data = await sodaNativeLyricRequest(trackId);
  const raw = data && data.lyric;
  const content = typeof raw === 'string'
    ? raw
    : String(raw && (raw.content || raw.lyric || raw.lrc || raw.text) || '');
  const translation = String(raw && (raw.translation || raw.tlyric || raw.translated_lyric || '') || '');
  const isYrc = /^\[\d+\s*,\s*\d+\]/m.test(content);
  return {
    provider: 'soda',
    id: trackId,
    lyric: content,
    tlyric: translation,
    yrc: isYrc ? content : '',
    source: content ? (isYrc ? 'soda-yrc' : 'soda-lrc') : 'soda-empty',
  };
}

async function handleSodaLike(id, like) {
  const trackId = String(id || '').trim();
  if (!trackId) return { provider: 'soda', ok: false, error: 'MISSING_SODA_TRACK_ID' };
  const media = [{ type: 'track', id: trackId }];
  const data = like
    ? await sodaPost('/luna/pc/me/collection/media', { media, scene: 'ClickHeartFromPlayer', collect_action: 1 })
    : await sodaPost('/luna/pc/me/collection/media/delete', { media });
  const liked = await getSodaLikedTrackIds(true);
  if (like) liked.add(trackId);
  else liked.delete(trackId);
  sodaLikedTracksCache.expiresAt = Date.now() + 30 * 1000;
  return { provider: 'soda', ok: true, id: trackId, liked: !!like, data };
}
...
function audioProxyHeadersFor(audioUrl, range) {
  ...
  try {
    ...
    if (host.includes('douyin.com') || host.includes('douyinvod.com') || host.includes('qishui.com')) {
      headers['User-Agent'] = `LunaPC/${SODA_VERSION_NAME}(${SODA_VERSION_CODE})`;
...
function readMp4Boxes(buffer, start, end) {
  ...
  while (offset + 8 <= limit) {
    ...
    boxes.push({ type, start: offset, size, headerSize, end: offset + size });
    offset += size;
  }
  return boxes;
}

const MP4_CONTAINER_TYPES = new Set(['moov', 'trak', 'mdia', 'minf', 'stbl', 'edts', 'dinf', 'udta', 'meta', 'sinf', 'schi']);

function mp4ChildBoxes(buffer, box) {
  let start = box.start + box.headerSize;
  if (box.type === 'meta') start += 4;
  return readMp4Boxes(buffer, start, box.end);
}

function mp4FindDescendant(buffer, box, type) {
  const stack = [box];
  while (stack.length) {
    const current = stack.pop();
    if (current.type === type) return current;
    // The synthetic root is not an MP4 box, but it still contains top-level boxes.
    if (current !== box && !MP4_CONTAINER_TYPES.has(current.type)) continue;
    const children = mp4ChildBoxes(buffer, current);
    for (let index = children.length - 1; index >= 0; index--) stack.push(children[index]);
  }
  return null;
}

function mp4TrackSampleEntry(buffer, stsd) {
  const entryCount = buffer.readUInt32BE(stsd.start + 12);
  if (!entryCount) return null;
  const start = stsd.start + 16;
  if (start + 8 > stsd.end) return null;
  const size = buffer.readUInt32BE(start);
  if (size < 8 || start + size > stsd.end) return null;
  return { start, size, type: buffer.toString('ascii', start + 4, start + 8) };
}

function parseMp4SampleSizes(buffer, box) {
  const defaultSize = buffer.readUInt32BE(box.start + 12);
  const count = buffer.readUInt32BE(box.start + 16);
  if (defaultSize) return Array(count).fill(defaultSize);
  const sizes = [];
  for (let index = 0, offset = box.start + 20; index < count; index++, offset += 4) {
    if (offset + 4 > box.end) throw new Error('SODA_MP4_STSZ_INVALID');
    sizes.push(buffer.readUInt32BE(offset));
  }
  return sizes;
}

function parseMp4ChunkOffsets(buffer, box) {
  const count = buffer.readUInt32BE(box.start + 12);
  const offsets = [];
  const is64 = box.type === 'co64';
  for (let index = 0, offset = box.start + 16; index < count; index++, offset += is64 ? 8 : 4) {
    if (offset + (is64 ? 8 : 4) > box.end) throw new Error('SODA_MP4_CHUNK_OFFSETS_INVALID');
    const value = is64 ? Number(buffer.readBigUInt64BE(offset)) : buffer.readUInt32BE(offset);
    if (!Number.isSafeInteger(value)) throw new Error('SODA_MP4_CHUNK_OFFSET_TOO_LARGE');
    offsets.push(value);
  }
  return offsets;
}

function parseMp4SampleToChunk(buffer, box) {
  const count = buffer.readUInt32BE(box.start + 12);
  const entries = [];
  for (let index = 0, offset = box.start + 16; index < count; index++, offset += 12) {
    if (offset + 12 > box.end) throw new Error('SODA_MP4_STSC_INVALID');
    entries.push({ firstChunk: buffer.readUInt32BE(offset), samplesPerChunk: buffer.readUInt32BE(offset + 4) });
  }
  return entries;
}
...
function buildMp4SampleOffsets(sampleSizes, chunkOffsets, sampleToChunk) {
  ...
  let sampleIndex = 0;
  let entryIndex = 0;
  for (let chunkIndex = 1; chunkIndex <= chunkOffsets.length && sampleIndex < sampleSizes.length; chunkIndex++) {
    while (entryIndex + 1 < sampleToChunk.length && sampleToChunk[entryIndex + 1].firstChunk <= chunkIndex) entryIndex++;
    const samplesPerChunk = sampleToChunk[entryIndex] && sampleToChunk[entryIndex].samplesPerChunk;
    if (!samplesPerChunk) throw new Error('SODA_MP4_STSC_MAPPING_INVALID');
    let offset = chunkOffsets[chunkIndex - 1];
    for (let sample = 0; sample < samplesPerChunk && sampleIndex < sampleSizes.length; sample++, sampleIndex++) {
      offsets.push(offset);
      offset += sampleSizes[sampleIndex];
    }
  }
  if (offsets.length !== sampleSizes.length) throw new Error('SODA_MP4_SAMPLE_COUNT_MISMATCH');
  return offsets;
}

function parseMp4SampleEncryption(buffer, box, ivSize) {
  const flags = buffer.readUInt32BE(box.start + 8) & 0x00ffffff;
  const count = buffer.readUInt32BE(box.start + 12);
  const entries = [];
  let offset = box.start + 16;
  for (let index = 0; index < count; index++) {
    if (offset + ivSize > box.end) throw new Error('SODA_MP4_SENC_INVALID');
    const iv = buffer.subarray(offset, offset + ivSize);
    offset += ivSize;
    const subSamples = [];
    if (flags & 0x2) {
      if (offset + 2 > box.end) throw new Error('SODA_MP4_SENC_SUBSAMPLE_INVALID');
      const count = buffer.readUInt16BE(offset);
      offset += 2;
      for (let sub = 0; sub < count; sub++) {
        if (offset + 6 > box.end) throw new Error('SODA_MP4_SENC_SUBSAMPLE_INVALID');
        subSamples.push({ clear: buffer.readUInt16BE(offset), encrypted: buffer.readUInt32BE(offset + 2) });
        offset += 6;
      }
    }
...
function decryptCencMp4(buffer, keyHex) {
  const key = Buffer.from(String(keyHex || ''), 'hex');
  if (key.length !== 16) throw new Error('SODA_CENC_KEY_INVALID');
  const root = { type: 'root', start: 0, end: buffer.length, headerSize: 0 };
  const moov = mp4FindDescendant(buffer, root, 'moov');
  if (!moov) throw new Error('SODA_MP4_MOOV_MISSING');
  const tracks = mp4ChildBoxes(buffer, moov).filter(box => box.type === 'trak');
  let target = null;
  for (const track of tracks) {
    const stsd = mp4FindDescendant(buffer, track, 'stsd');
    const entry = stsd && mp4TrackSampleEntry(buffer, stsd);
    if (entry && entry.type === 'enca') {
      target = { track, stsd, entry };
      break;
    }
  }
  if (!target) throw new Error('SODA_CENC_AUDIO_TRACK_MISSING');

  const stsz = mp4FindDescendant(buffer, target.track, 'stsz');
  const stsc = mp4FindDescendant(buffer, target.track, 'stsc');
  const stco = mp4FindDescendant(buffer, target.track, 'stco') || mp4FindDescendant(buffer, target.track, 'co64');
  const senc = mp4FindDescendant(buffer, target.track, 'senc');
  if (!stsz || !stsc || !stco || !senc) throw new Error('SODA_CENC_TABLES_MISSING');

  const tencOffset = buffer.indexOf('tenc', target.entry.start, 'ascii');
  if (tencOffset < 4 || tencOffset + 15 >= target.entry.start + target.entry.size) throw new Error('SODA_CENC_TENC_MISSING');
  const ivSize = buffer.readUInt8(tencOffset + 11);
  if (ivSize !== 8 && ivSize !== 16) throw new Error('SODA_CENC_IV_INVALID');

  const sizes = parseMp4SampleSizes(buffer, stsz);
  const offsets = buildMp4SampleOffsets(sizes, parseMp4ChunkOffsets(buffer, stco), parseMp4SampleToChunk(buffer, stsc));
  const encryption = parseMp4SampleEncryption(buffer, senc, ivSize);
  if (encryption.length !== sizes.length) throw new Error('SODA_CENC_SAMPLE_COUNT_MISMATCH');

  const output = Buffer.from(buffer);
  for (let index = 0; index < sizes.length; index++) {
    const start = offsets[index];
    const end = start + sizes[index];
    if (start < 0 || end > output.length) throw new Error('SODA_CENC_SAMPLE_RANGE_INVALID');
    const iv = encryption[index].iv.length === 16 ? encryption[index].iv : Buffer.concat([encryption[index].iv, Buffer.alloc(8)]);
    const decipher = crypto.createDecipheriv('aes-128-ctr', key, iv);
    const entry = encryption[index];
    if (!entry.subSamples.length) {
      decipher.update(output.subarray(start, end)).copy(output, start);
      decipher.final();
      continue;
    }
    let cursor = start;
    for (const subSample of entry.subSamples) {
      cursor += subSample.clear;
      if (subSample.encrypted) {
        decipher.update(output.subarray(cursor, cursor + subSample.encrypted)).copy(output, cursor);
        cursor += subSample.encrypted;
      }
    }
    decipher.final();
  }

  const sinfTypeOffset = buffer.indexOf('sinf', target.entry.start + 8, 'ascii');
  const sinf = sinfTypeOffset >= target.entry.start + 4 && sinfTypeOffset + 4 <= target.entry.start + target.entry.size
    ? { start: sinfTypeOffset - 4 }
    : null;
  const frmaTypeOffset = buffer.indexOf('frma', target.entry.start + 8, 'ascii');
  const originalFormat = frmaTypeOffset >= target.entry.start + 4 && frmaTypeOffset + 8 <= target.entry.start + target.entry.size
    ? buffer.toString('ascii', frmaTypeOffset + 4, frmaTypeOffset + 8)
    : '';
  if (!/^[\x20-\x7e]{4}$/.test(originalFormat)) throw new Error('SODA_MP4_FRMA_MISSING');
  // Restore the original codec entry (for example fLaC) instead of relabelling
  // every encrypted stream as AAC. Chromium rejects an mp4a entry with FLAC config.
  output.write(originalFormat, target.entry.start + 4, 'ascii');
  for (const box of [sinf, senc, mp4FindDescendant(buffer, target.track, 'saiz'), mp4FindDescendant(buffer, target.track, 'saio')]) {
    if (box) output.write('free', box.start + 4, 'ascii');
  }
  return output;
}

async function getSodaDecryptedAudio(audioUrl) {
  const entry = sodaAudioDecryptions.get(String(audioUrl));
  if (!entry || entry.expiresAt <= Date.now()) return null;
  if (entry.buffer) return entry;
  if (!entry.loading) {
    entry.loading = (async () => {
      const upstream = await fetch(audioUrl, { headers: audioProxyHeadersFor(audioUrl) });
      if (!upstream.ok) throw new Error(`SODA_AUDIO_UPSTREAM_${upstream.status}`);
      entry.contentType = audioContentTypeForUrl(audioUrl, upstream.headers.get('content-type'));
      entry.buffer = decryptCencMp4(Buffer.from(await upstream.arrayBuffer()), entry.key);
      return entry;
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/api/weather/reverse-location') {
    ...
    if (!['127.0.0.1', '::1', '::ffff:127.0.0.1'].includes(remoteAddress)) {
      sendJSON(res, { ok: false, error: 'LOCAL_ONLY', location: null }, 403);
      return;
    }
    try {
      ...
      console.warn('[WeatherReverseLocation]', err.message);
      sendJSON(res, { ok: false, error: err.message, location: null }, 500);
    }
    return;
  }

  if (pn === '/api/weather/system-location') {
    const remoteAddress = String(req.socket && req.socket.remoteAddress || '');
    if (!['127.0.0.1', '::1', '::ffff:127.0.0.1'].includes(remoteAddress)) {
      sendJSON(res, { ok: false, error: 'LOCAL_ONLY', location: null }, 403);
      return;
    ...
    try {
      const location = await fetchWindowsWeatherLocation();
      try { Object.assign(location, await reverseWeatherLocation(location.latitude, location.longitude)); }
      catch (err) { console.warn('[WeatherSystemReverseLocation]', err.message); }
      sendJSON(res, { ok: true, location });
    } catch (err) {
      console.warn('[WeatherSystemLocation]', err.message);
      sendJSON(res, { ok: false, error: err.message, location: null }, 500);
    }
    return;
  }
  ...
  if (pn === '/api/soda/version') {
    sendJSON(res, { provider: 'soda', supported: true, version: SODA_VERSION_NAME, versionCode: SODA_VERSION_CODE });
    return;
  }

  if (pn === '/api/soda/login/status') {
    try {
      sendJSON(res, await getSodaLoginInfo());
    } catch (err) {
      sendJSON(res, { provider: 'soda', loggedIn: false, version: SODA_VERSION_NAME, error: err.message }, 500);
    }
    return;
  }
  ...
  if (pn === '/api/soda/login/cookie') {
    try {
      const body = await readRequestBody(req);
      const normalized = normalizeCookieHeader(body.cookie || body.data || body.text || '');
      const candidate = parseCookieString(normalized);
      if (!(candidate.sessionid || candidate.sessionid_ss || candidate.sid_tt)) {
        sendJSON(res, { provider: 'soda', loggedIn: false, error: 'INVALID_SODA_SESSION' }, 400);
        return;
      }
      ...
      if (!info.loggedIn) {
        saveSodaCookie('');
        sendJSON(res, { provider: 'soda', loggedIn: false, error: 'SODA_SESSION_EXPIRED' }, 400);
        return;
      }
      sendJSON(res, Object.assign(info, { saved: true }));
    } catch (err) {
  ...
  if (pn === '/api/soda/user/playlists') {
    try {
      sendJSON(res, await handleSodaUserPlaylists());
    } catch (err) {
  ...
  if (pn === '/api/soda/playlist/tracks') {
    try {
      sendJSON(res, await handleSodaPlaylistTracks(url.searchParams.get('id') || url.searchParams.get('playlist_id') || ''));
    } catch (err) {
  ...
  if (pn === '/api/soda/search') {
    try {
      ...
      sendJSON(res, { provider: 'soda', songs: await handleSodaSearch(keywords, limit) });
    } catch (err) {
  ...
  if (pn === '/api/soda/song/url') {
    try {
      sendJSON(res, await handleSodaSongUrl(url.searchParams.get('id') || url.searchParams.get('track_id') || '', url.searchParams.get('vid') || '', url.searchParams.get('quality') || 'hires'));
    } catch (err) {
  ...
  if (pn === '/api/soda/lyric') {
    try {
      sendJSON(res, await handleSodaLyric(url.searchParams.get('id') || url.searchParams.get('track_id') || ''));
    } catch (err) {
      console.error('[SodaLyric]', err);
      sendJSON(res, { provider: 'soda', lyric: '', tlyric: '', yrc: '', error: err.message }, 500);
    }
    return;
  }

  if (pn === '/api/soda/likes/check') {
    try {
      const ids = String(url.searchParams.get('ids') || '').split(',').map(id => id.trim()).filter(Boolean);
      const likedIds = await getSodaLikedTrackIds();
      const liked = {};
      ids.forEach(id => { liked[id] = likedIds.has(String(id)); });
      sendJSON(res, { provider: 'soda', liked, playlistId: sodaLikedTracksCache.playlistId });
    } catch (err) {
      console.error('[SodaLikeCheck]', err);
      sendJSON(res, { provider: 'soda', error: err.message, liked: {} }, 500);
    }
    return;
  }
  ...
  if (pn === '/api/soda/like') {
    try {
      ...
      const rawLike = String(url.searchParams.get('like') || 'true').toLowerCase();
      const like = rawLike !== 'false' && rawLike !== '0';
      sendJSON(res, await handleSodaLike(id, like));
    } catch (err) {
      console.error('[SodaLike]', err);
      sendJSON(res, { provider: 'soda', ok: false, error: err.message }, 500);
    }
    return;
  }
  ...
  if (pn === '/api/audio') {
    try {
      ...
      // 汽水返回的是 CENC 加密 MP4。浏览器原生 audio 不支持该私有解复用器，
      // 因此命中汽水播放 URL 时先在服务端解密，再按标准 Range 返回缓存的字节。
      const decrypted = await getSodaDecryptedAudio(audioUrl);
      if (decrypted && decrypted.buffer) {
        ...
        const body = decrypted.buffer.subarray(start, end + 1);
        const out = {
        ...
        if (range) out['Content-Range'] = `bytes ${start}-${end}/${total}`;
        res.writeHead(range ? 206 : 200, out);
        res.end(body);
        return;
      }
...
server.stopSodaBridge = () => stopSodaBridgeProcess(sodaBridgeProcess);
